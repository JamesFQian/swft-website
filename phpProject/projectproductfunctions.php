<?php 
/*
*  projectproductfunctions.php
*  project
*  Morning class fall 2016
*  James Qian
*/


function get_product_name($dbc, $product_id)
{
	$query = "SELECT name FROM proj_products WHERE id = $product_id";
	$result = mysqli_query($dbc, $query);
	$row = mysqli_fetch_array($result);
	return $row['name'];
}

function get_price($dbc, $product_id)
{
	$query = "SELECT price FROM proj_products WHERE id = $product_id";
	$result = mysqli_query($dbc, $query);
	$row = mysqli_fetch_array($result);
	return $row['price'];
}
...
function get_order_total($dbc)
{
	...
	for($i = 0; $i < $max; $i++)
	{
		...
		$price = get_price($dbc,$product_id);
		$sum += $price * $quantity;
	}
	return $sum;
}
